...
export const PROMPT_TYPES = ["auto", "frequency", "percentile", "samples", "normal"] as const;
export type PromptType = (typeof PROMPT_TYPES)[number];

const FORMATS: Record<Exclude<PromptType, "auto">, string> = {
  frequency: `"frequency" - ordered categories with the share of the population in each,
...
(Numbers may be percentages or raw counts; they don't need to sum to 100.)`,
  percentile: `"percentile" - known values and the percentile at each. Keys may be numbers
...
"% of people below this value", or "top" when it is "top X%":
  ...
  samples: `"samples" - a raw list of observed numbers, in any order:
  {"name": "team-salaries", "type": "samples", "unit": "USD",
   "data": [72000, 85000, 91000, 104000, 150000]}`,
  normal: `"normal" - a bell curve given by mean and standard deviation:
  {"name": "iq", "type": "normal", "data": {"mean": 100, "std": 15}}`,
};

/** A prompt asking an AI assistant for the distribution of `topic` as JSON. */
export function buildPrompt(topic: string, kind: PromptType = "auto"): string {
  const formats =
    kind === "auto"
      ? "Choose whichever format best fits the data:\n\n" +
        (["frequency", "percentile", "samples", "normal"] as const).map((k) => FORMATS[k]).join("\n\n")
      : "Use this format:\n\n" + FORMATS[kind];
  return `I need the population distribution of: ${topic}
